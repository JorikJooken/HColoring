...
bool canBeColoredRec(long long unvisited)
{
    if(unvisited==0)
    {
        for(int i=0; i<n; i++)
        {
            traverse_through_neighbors_nest1(adjacencyList[i],neighbor)
            {
                printf("Edge between %d and %d has color %d\n",i,neighbor,color[i][neighbor]);
            } 
        }
        return true;
    }
...
// It will output all graphs that are not 2C3-colorable
int main()
{
    AArray[0][0]=0;
    AArray[0][1]=1;
